 ...
 */
class mapper_Comment extends mapper_Mapper
{
    private $selectStmt;
    ...
    public function __construct() {
        parent::__construct();
        $this->selectStmt    = self::$PDO->prepare( "SELECT * FROM post WHERE id_post=?" );
        $this->selectAllStmt = self::$PDO->prepare("SELECT id_post, name, create_date, content, id_user FROM post" );
        $this->insertStmt    = self::$PDO->prepare("INSERT INTO post (name, create_date, content, id_user) values(?,?,?,?)");
        $this->updateStmt    = self::$PDO->prepare("UPDATE post SET name=?,id=? WHERE id=?");
    }
